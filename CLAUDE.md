...
Use `postgresql.ENUM`, not the generic `sa.Enum`. `sa.Enum(..., create_type=False)` does not reliably forward the flag to the postgres dialect when it adapts, so SQLAlchemy still emits `CREATE TYPE` at `op.create_table` time and you get `DuplicateObject: type "user_role" already exists` on a fresh DB where `ensure_enum_exists` already created it. `postgresql.ENUM(..., create_type=False)` honors the flag. To add a value later, use `add_enum_value("user_role", "guest")`.
